...
вызывает в голове комические образы мужчин с халкообразными торсом, балансирующим на тощих ногах.
...
выглядит смешной, или может быть потому, что в ней только для шутки?
...
подумать, что годы спустя эта концепция прочно укорениться в моем образе мышления, когда я начал
...
имеем свойство впадать, и из чего нам стоит выбираться, прикладывая свои усилия.
...
Давайте по частям. Я создаю две структуры для хранения наших значений. Следующим шагом я убеждаюсь,
что они реализуют нечто, называемое `fmt::Display`. В C++ мы бы перегрузили оператор `<<` для
`ostream`. Результат был бы аналогичным. Теперь я могу вызывать функцию печати, передавая свои
структуры напрямую.

Это уже половина истории.

Дальше у нас возникает функция `print_me`. Эта функция обобщенная и принимает все что угодно в случае
если оно умеет `fmt::Display`. К счастью, мы только что убедились что наши структуры так умеют.

Все остальное просто. Мы создаем несколько экземпляров и передаем их на печать в `print_me`.
...
до использования обобщений, и пошли бы по пути объектно-ориентированного полиморфизма:
...
без каких-либо предупреждений для разработчика, что то что он делает "является скорее всего
плохой идеей™".
...
Хоть это не совсем очевидно, этот код использует пробрасывание исключений. Вся
...
Это означает, что если у `load_header` были какие-либо проблемы при вызове
`file.read_u32`, то функция вернет `io::Error`. Далее, тоже самое произойдет в `load_audio`, и из нее
будет возвращена та же самая ошибка. И так далее до тех пор, пока вызывающая функция наконец не
обработает ошибку.
...
можете читать их сколько вам заблагорассудиться из любого количества потоков.
...
Гораздо труднее писать безопасный код, основываясь на том, что небезопасно с самого начала.

Хоть Rust и дает возможность выбора, он склоняет вас не позволить выстрелить себе в ногу.
