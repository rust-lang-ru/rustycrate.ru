...
> **Замечание**: это перевод [статьи](https://llogiq.github.io/2015/11/27/from-into.html).
...
Если вы долго программируете на Rust, вы должно быть заметить, что мы,
...
является действительным типом (не типажом). Также у нас есть тип Blob,
...
автоматического создания ссылок, что в Rust не реализовано (по понятным
причинам, хочу добавить). Проблема тут в различии между Bar and &Bar. В примере
попроще этого бы не произошло. Увы, в реальности все не так просто.
