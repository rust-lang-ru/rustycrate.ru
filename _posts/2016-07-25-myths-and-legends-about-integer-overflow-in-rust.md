...
categories: обучение
excerpt: >
    В статье описывается как Rust обрабатывает переполнение целых чисел.
---
...
но также старается помочь программисту избежать остальных проблем: [утечек памяти](https://habrahabr.ru/post/281370/), 
[игнорировании ошибок](https://doc.rust-lang.org/std/result/#results-must-be-used) и, 
...
использованию [дополнения до двух](https://en.wikipedia.org/wiki/Two's_complement) 
...
защитой от нарушения безопасности работы с памятью может легко приводить к повреждению 
памяти. Впрочем, в более заботящихся о безопасности языках, таких как Rust, это 
...
Проще говоря, код наподобие `max(x - y, z)` периодически попадается и может давать 
...
Текущий статус определился в [RFC 560](https://github.com/rust-lang/rfcs/pull/560):
...
часто встречаются, когда `x` известен во время компиляции, а значит всё выражение заменено 
константой.
...
Благодаря усилиям отдельных людей, RFC принял современную форму: в результате переполнения 
...
должно происходить): в этом случае `pushq`/`pop`/`movl` работа с регистрами не 
обязательна. Я полагаю, что даже без встраивания `pushq`/`popq` управление стеком не 
...
параллелизму на уровне инструкций. x86 ISA разрешает разыменование сложных вычислений с 
указателями: общая форма `A(r1, r2, B)` (в AT&T синтаксисе), что эквивалентно 
...
устанавливает CPU флаг, свидетельствующий об этом.

Однако, ещё больший удар по производительности наносит то, что проверки переполнения 
препятствуют другим оптимизациям. Во-первых, проверки сами по себе сериализуют код 
(препятствуя таким вещам, как размотка и переустановка циклов). Во-вторых, паника и 
...
Таким образом, даже если проверки переполнения включены в релизном режиме, потери 
...
Все усилия по разработке, обсуждению и реализации этой схемы обработки целочисельных 
