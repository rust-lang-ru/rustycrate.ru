...
excerpt: В данной статье представлен перевод официального руководства по достаточно молодой библиотеке 
`futures-rs`, у которой есть все предпосылки стать важной частью экосистемы `Rust`.
...
Если создадим файл с таким содержанием по пути `src/main.rs` и запустим команду `cargo run`, увидем HTML
...
Здесь создадим [цикл событий](http://alexcrichton.com/futures-rs/futures_mio/struct.Loop.html#method.new), в
...
[Получаем хэндл](http://alexcrichton.com/futures-rs/futures_mio/struct.Loop.html#method.handle) цикла событий и
...
Здесь используем метод [and_then](http://alexcrichton.com/futures-rs/futures/trait.Future.html#method.and_then)
...
Если запустить программу с этого места, вы удивитесь, так как ничего не произойдёт, 
...
выполнение future](http://alexcrichton.com/futures-rs/futures/struct.Task.html#method.run). Цикл событий будет 
выполняться, пока не будет получен результат.
...
высокоуровневые абстракции вокруг futures. Знание, того, как futures работают под капотом будет полезным.
...
Взглянем на тип возвращаемого значения `Poll`:
...
Метод `task::park` является основной точкой входа доставки уведомлений. Эта функция возвращает 
...
Поток `clients` производит сокеты постоянно. Это иллюстрирует работу сокет серверов, как они обычно принимают клиентов 
...
Каждая часть реализует типаж `Send` и по отдельности является владельцом сущности. Часто использовать эту реализацию, 
...
вступает в силу только при использовании `Box`.
...
Достоинством такого подхода является, то, что он не требует выделения памяти для `Box` и по прежнему мааксимально гибок.
...
Достоинством данного подхода является его эргономичность в отличие от пользовательских типов future, а так же отсутствие 
...
Вкратце `Task` - это сущность, которая на самом деле управляет высокоуровневыми вызовами функции `poll`. Её основной 
метод, [run](http://alexcrichton.com/futures-rs/futures/task/struct.Task.html#method.run), занимается именно этим.
...
Сами по себе задачи, как правило, не создаются вручную, а производятся путём вызова функции 
[forget](http://alexcrichton.com/futures-rs/futures/trait.Future.html#method.forget).
Эта функция типажа [Future](http://alexcrichton.com/futures-rs/futures/trait.Future.html) создаёт новую 
[Task](http://alexcrichton.com/futures-rs/futures/task/struct.Task.html) и после запрашивает запуск future, 
последовательно передавая всю цепочку скомбинированных futures.
...
с доступом к стэку, доступному посредством блокирующего ввода/вывода, асинхронное вычисление 
запускает отдельные futures, с доступом к `Task`, которое сохраняется в течении всего времени выполнения.
...
Оба эти решения относительно тяжеловесны, так посмотрим сможем ли мы сделать лучше.
...
является то, что он реализует типаж `Send` независимо от того, какие данные будут храниться в цикле событий.

Один из ключевых методов типажа `Future` `forget` требует future, реализующий типаж `Send`. Это не 
всегда возможно для future, но `LoopData<F: Future>` реализует `Send`, и типаж `Future` реализован напрямую для 
`LoopData<F>`. Это означает, что если future не реализует типаж `Send`, он может "превратиться" в таковой, 
"прикрепляя" его в цикл событий и предварительно обернув в `LoopData`.
...
В случае, если был возвращён `None`, future может вернуться в цикл событий, чтобы продолжить выполнение.
Чтобы гарантировать такое поведение,
[executor](https://tokio-rs.github.io/tokio-core/tokio_core/struct.LoopData.html#method.executor), ассоциированый с 
данными, может быть получен и передан в функцию 
[Task::poll_on](http://alexcrichton.com/futures-rs/futures/task/fn.poll_on.html).
Для этого требуется, чтобы задача опрашивалась сама по указанному исполнителю, который в данном случае будет 
выполнять запрос на опрос цикла событий, где данные могут быть доступны.

`LoopData` создаётся с помощью двух методов:
...
тип возвращаемого значения: этот метод вернёт future, который вернёт значение типа 
[LoopData](`https://tokio-rs.github.io/tokio-core/tokio_core/struct.LoopData.html`).

Локальные данные задачи и данные цикла событий обеспечивают возможность лёгкого управления разделяемым 
состоянием (отправляемыми и не-отправляемыми данные) среди множества futures.