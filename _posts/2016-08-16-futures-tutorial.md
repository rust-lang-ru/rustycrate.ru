...
метод, [run](http://alexcrichton.com/futures-rs/futures/task/struct.Run.html#method.run), занимается именно этим.
Внутренне `Task` устроен таким образом, что если 
[unpark](http://alexcrichton.com/futures-rs/futures/task/struct.Task.html#method.unpark) вызван из множества 
...
- макрос `task_local!`, очень похожий на макрос `thread_local!` из стандартной библиотеки. Данные инициализирующиеся 
 этим путём будут лениво инициализироваться при первом доступе к `Task`, а уничтожаться они будут, когда `Task` будет 
 уничтожен;
 
 - структура [TaskRc](http://alexcrichton.com/futures-rs/futures/task/struct.TaskRc.html) обеспечивает возможность 
 создание подсчёта ссылок на данные, которые могут быть доступны в соответствующей задаче. Она может быть клонирована, 
 так же как и `Rc`.
