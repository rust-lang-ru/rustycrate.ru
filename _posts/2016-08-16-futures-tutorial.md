...
[unpark](http://alexcrichton.com/futures-rs/futures/struct.TaskHandle.html#method.unpark) указывает, что future
...
Поток `clients` производит сокеты постоянно. Это иллюстрирует работу сокет серверов по приему клиентов в цикле и 
направлению их в остальную часть системы обработки.
...
`LoopData<F>`. Это означает, что если future не реализует типаж `Send`, он может стать таковым, после оборачивания его 
в `LoopData`.
...
опрашивать цикл событий, где данные могут быть доступны.
...
состоянием среди множества futures.