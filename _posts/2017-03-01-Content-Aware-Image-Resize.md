---
title: "Изменение размеров картинки по содержимому"
...
# Изменение размеров картинки по содержимому.
“Изменение размеров картинки по содержимому(Content Aware Image Resize)”, “Жидкое растяжение(liquid resizing)”, “перенастройка(retargeting)” или “вырезание шва(seam carving)”, относятся к методу изменения размеров изображения, где можно вставлять или удалять *швы*, или “наименее важные пути”, для уменьшения или наращивания изображения. Об этой идее я узнал [На YouTube, авторов](https://www.youtube.com/watch?v=qadw0BRKeMk), авторы Shai Avidan и Ariel Shamir.
В этой статье будет рассмотрена простая пробная реализация идеи изменения размеров изображения на основе содержания, естественно на языке Rust :)

В качестве подопытной картинки, я <a name='ref1ret'></a>поискал по запросу[[1]](#ref1)  `"sample image"`, и нашел <a name='ref2ret'></a>её[[2]](#ref2):

{% img '2017-03-01-Content-Aware-Image-Resize/sample-image.jpeg' alt:'sample image' magick:resize:800 %}

# Набросок - нисходящий подход.
...
# image
Библиотека [`image`](https://crates.io/crates/image) от разработчиков “Piston” кажется подойдет, поэтому мы добавим в наш `Cargo.toml` запись: `image = "0.12"`. Быстрый поиск в документации это все, что требуется для нас, чтобы написать функцию загрузки изображения:
...
Суть алгоритма состоит в поиске пути P=p1… p6
от одной из верхних ячеек G1i в одну из нижних G6j, так, чтобы минимизировать ∑1≤i≤6pi. Это может быть сделано путем создания новой таблицы S
...
Также обратите внимание, что есть два пути, которые в сумме дают 8 (две нижние ячейки различаются этими двумя путями).
...
impl DPTable {
    fn from_gradient_buffer(gradient: &GradientBuffer) -> Self {
        ...
        // Получим градиент[h][w], сохранив как другой тип
