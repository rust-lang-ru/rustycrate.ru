...
зависимостей, что бы компилятор мог проверить типы и тому подобное. Это так же полезно для
...
Компилятор объясняет проблему и даёт полезный совет. Что ж давайте попробуем им воспользоваться, изменим код
добавив в него `'a` и попробуем скомпилировать снова.
...
Для кода выше было бы следующее сообщение об ошибке:
