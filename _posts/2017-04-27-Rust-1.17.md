...
Теперь `cargo build` будет собирать наш `postgres-tool`, только если у нас включены две эти фичи, и поэтому мы можем написать нормальный `fn main` без всяких нагромождений вроде `cfg`.
