...
- умножается длина на ширину. Но в идеале мы не хотим хранить в квадрате
...
Отметим ещё раз компромисс:
  - Если объекты находятся в одной ветви иерархии, можно сделать типаж,
    определяющий общие черты этих объектов. Тогда можно сделать типаж с
  ...
  - Если объекты разнородны и находятся в разных ветвях иерархии, нельзя сделать
    такой типаж. Нужно реализовывать типаж для каждой структуры напрямую.
...
их [в соответствующей теме](https://forum.rustycrate.ru/woo) на форуме.
