...
наших пользователей на Windows. На этой ОС Rust использует для сборки `link.exe`,
...
Объединения похожи на `enum` (`Перечисления`), но они не имеют "тегов" в отличие
от последних. "Тег" в `enum` позволяет узнать во время исполнения, какой вариант
