...
- Что такое легковесные потоки (**_threads_**)?
...
должен выделить большой стек (**_stack_**), настроить поток, используя набор
системных вызовов. Ко всему прочему переключение контекста тоже затратно.

Конечно, тысячи одновременно работающих потоков не будет: вы имеете
ограниченное число ядер (**_core_**), и **в любой момент времени только один
поток будет исполняться на этом ядре**.
...
будет завершена, то есть когда исходный поток будет "разблокирован"
(**_unblocked_**). Вот так вы решаете такие задачи в Rust, когда не
используете `Tokio` и подобные ей библиотеки - запускаете миллион потоков и
позволяете ОС самостоятельно планировать (**_schedule_**) запуск и завершение
потоков в зависимости от ввода/вывода.
...
---
## Модель многопоточности основанная на легковесных потоках (_lightweight threads_).
...
Так, **Go** имеет легковесные потоки, называемые горутинами (**_goroutines_**).
...
Горутина является примером легковесного потока. ОС ничего о них не знает, она
видит N потоков, которые находятся в распоряжении системы выполнения Go (**_Go
runtime_**, далее СВ Go).

Горутины - это легковесные потоки, о которых ОС не имеет никакой информации.
ОС видит только N потоков, принадлежащих среде исполнения (runtime) Go, и Go
...
Go использует сегментированные стеки (_segmented stacks_). Каждый поток
требует сравнительно большой стек, потому что большинство языков
программирования (включая С) поддерживают только непрерывные стеки с
постоянным размером. Это позволяет быть уверенным в том, что указатели на стек
никогда не будут испорчены перевыделением памяти.
...
что в Go есть GC, а поэтому в любом случае знает про все указатели и может при
необходимости перезаписывать те из них, что указывают на стек.
...
Rust _поддерживал_ легковесные/green потоки (Я верю, что он использовал
...
потоку не будет позволено продолжать выполнение (он стал заблокированным) до
...
же ничего не происходит, поток планировщика может уйти в сон (_sleep_), вызвав
`epoll`, ожидая до тех пор, пока ОС не разбудит его (когда одна из операций
...
платформо-независимой обёрткой (_wrapper_) над неблокирующим вводом/выводом и
такими инструментами, как `epoll` (GNU/Linux), `kqueue` (FreeBSD), и т. д. Это
...
в цепочку несколько сущностей, например, операции ввода/вывода. C футурами на цепочке
...
футуры. Tokio имеет главный цикл обработки событий (**_event loop_**), в
который вы можете передавать замыкания, возвращающие футуры. Этот цикл будет
выполнять все замыкания, которые вы ему передадите, используя `mio` для
выяснения того, какие футуры, могут прогрессировать, и продвигать их далее
(вызывая `poll()`).
...
ввод/вывод. Каждый раз когда одна из текущих задач будет заблокирована на
операции ввода/вывода, цикл будет подгружать новую задачу. Ключевым отличием
...
использовать каналы (_channels_) для их координирования, что не сложно.
...
пул потоков и последовательно (pipeline) обрабатывать данные, а async/await
является здесь хорошим подспорьем.

Обобщим: все эти компоненты, несмотря на небольшое количество избыточного
(_boilerplate_) кода, дают почти то же самое, что и горутины в Go. А так как
генераторы (а следовательно, и `async/await`) хорошо сосуществуют c
анализатором заимствований (**_borrow checker_**), то инварианты безопасности,
которые поддерживает Rust, по-прежнему в силе, и мы получаем "безбоязненную
конкурентность" для програм, которые выполняют большой объем задач
ввода/вывода.
