...
title: "В Rust вектора - это значения"
...
экспериментальной библиотеке персистентных коллеций - [DVec], которая делает
...
и не имеют своей уникальности. Посмотрите на этот работающий с числами JS-код:
```
function foo() {
    let x = 0;
    let y = x;
...
при сборщике мусора неизместно, сколько указателей ссылается на участок памяти.
**Это верно при наличии сборщика мусора**, ибо дейстия, подобные
...
не обязательно должны иметь тот же самый интерфейс**. Например, я написал
реализацию персистентного вектора - библиотеку [dogged]. Библиотека
