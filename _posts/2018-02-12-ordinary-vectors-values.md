...
в функциональные языки, хорошо применяется это и в написании параллельных
програм.

## Как работают персистентные коллекции?
Не буду вдаваться в подробности какого-то конкретного дизайна, который
вероятнее всего реализован с использованием дерева. Например, имея
...
занимает примерно O(log n) - мы должны склонировать некоторый лист и изменить
...
Несколько замечаний:
- если на вектор имеется только одна ссылка и вы это знаете, вы часто можете
избежать этих клонирований и просто менять дерево на месте. Позже я расскажу
об экспериментальной библиотеке персистентных коллеций - [DVec], которая делает
как раз это. Это сложно реализовать в языке со сборкой мусора, потому что вы
никогда не знаете, сколько ссылок имеется на вашу коллекцию.
- есть много других дизайнов персистентным коллекий, которые имеют своей
целью оптимизацию под конкретные случаи использования. Например, [в данной
работе][paper] приводится дизайна, подходящий для Prolog'о-подобных приложений.
Данный дизайн производит "под капотом" изменения, для того, чтобы можно было
делать вставку за O(1), хотя за пользовательским API это не видно. Разумеется,
за эти быстрые вставки нужно платить: использование старых копий структуры
данных является ресурсозатратным.
...
и не имеют своей идентичности. Посмотрите на этот работающий с числами JS-код:
```
function foo() {
    let x = 0;
    let y = x;
...
function foo() {
    ...
    // имеют связь, которая не очевидна на первый взгляд.
    ...
}
```
...
> Предоставление общего доступа и изменяемость: хорошо сами по себе,
> отвратительны вкупе.
...
при сборщике мусора неизместно, скольок указателей ссылается на участок памяти.
**Однако это еще более верно при наличии сборщика мусора**, ибо дейстия, подобные
...
будет передано другой переменной, поэтому она (`x`) не может быть больше
использована.
...
через разные указатели - вы можете заменить его числом, и оно будет вести себя так же.

**Из всего вышесказанного следует, что персистентные коллекции в Rust имеют
тот же самый интерфейс, что и обычные**. Например, я написал реализацию
персистентного вектор - библиотеку [dogged]. Библиотека предоставляет тип
[DVec], который основан на
[персистентном векторе, предоставляемом Clojure][pvec_by_cloj]. Если вы
...
использование `DVec` - вы много раз клонируете вектор, и они содержат в себе
...
те же утверждения, хотя и с несколько другой стороны. Он говорит, что вы
можете предоставлять персистентный интерфейс (например, метод
`vec.add(element)`), который возвращает новый вектор. Однако если вы
используете линейные типы, вы можете реализовать это как императивную
структуру данных, и никто об этом не узнает.

Изучая `DVec`, я понял, что очень удобно иметь персистентный вектор, который
предлагает такой же интерфейс, как и обычный. Например, было очень легко
изменить основанную на векторе библиотеку [ena] так, чтобы она работала
...
Я думаю, что есть еще одно основание для того, чтобы кто-то целенаправленно
занялся персистентными коллециями. В то время как одновременное предоставление
доступа и изменяемость могут представлять опасность, иногда это бывает нужно,
...
которые, однако, имеют неэффективную операцию `clone`.
